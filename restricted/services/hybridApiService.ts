...
import { DashboardData, DashboardMetric } from '../types/dashboardTypes';
...
export const fetchHybridDashboardData = async (): Promise<DashboardData> => {
  console.log('Fetching dashboard data with hybrid approach...');
  
  const apiConfig = ConfigService.getApiConfig();
  const responseMapping = ConfigService.getResponseMapping();

  // If no configuration is available, return mock data
  ...
  try {
    console.log('Making API request to:', `${apiConfig.baseUrl}${apiConfig.endpoints.dashboard}`);
    ...
    console.error('Failed to fetch dashboard data from API, falling back to mock data:', error);
    return mockDashboardData;
  }
};
